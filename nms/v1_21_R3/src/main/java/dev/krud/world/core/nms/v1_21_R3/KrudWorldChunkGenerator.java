...
import dev.krud.world.engine.framework.ResultLocator;
...
import dev.krud.world.engine.object.KrudWorldStructurePopulator;
...
import dev.krud.world.util.reflect.WrappedReturningMethod;
...
public class KrudWorldChunkGenerator extends CustomChunkGenerator {
    ...
    private final KrudWorldStructurePopulator populator;

    public KrudWorldChunkGenerator(ChunkGenerator delegate, long seed, Engine engine, World world) {
        super(((CraftWorld) world).getHandle(), edit(delegate, new CustomBiomeSource(seed, engine, world)), null);
        this.delegate = delegate;
        this.engine = engine;
        this.populator = new KrudWorldStructurePopulator(engine);
        var dimension = engine.getDimension();

        KSet<KrudWorldJigsawStructure> placements = new KSet<>();
        ...
        for (var s : placements) {
            try {
                ...
                if (set == null) {
                    KrudWorld.error("Could not find structure tag: " + raw);
                    continue;
                }
                for (var holder : set) {
                ...
                KrudWorld.error("Failed to load structure: " + s.getLoadKey());
                e.printStackTrace();
            }
        }
    }

    private void addAll(KList<KrudWorldJigsawStructurePlacement> placements, KSet<KrudWorldJigsawStructure> structures) {
