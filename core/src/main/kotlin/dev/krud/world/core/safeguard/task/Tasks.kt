...
import dev.krud.world.core.safeguard.Mode.*
import dev.krud.world.core.safeguard.task.Diagnostic.Logger.*
import dev.krud.world.core.safeguard.task.Task.Companion.of
...
private val incompatibilities by task {
    ...
    if (plugins.isEmpty()) STABLE.withDiagnostics()
        ...
        if ("Stratos" in plugins) diagnostics.addAll(
            ...
            ERROR.create("- KrudWorld is not compatible with other worldgen plugins.")
        )
...
private val version by task {
    val parts = KrudWorld.instance.description.version.split('-')
    ...
    if (INMS.get() !is NMSBinding1X) STABLE.withDiagnostics()
        ...
        ERROR.create("- KrudWorld only supports $minVersion > $maxVersion")
    )
}
...
private val dimensionTypes by task {
    ...
    if (!INMS.get().missingDimensionTypes(*keys.toTypedArray())) STABLE.withDiagnostics()
        ...
        ERROR.create("- Required KrudWorld dimension types were not loaded."),
...
private val diskSpace by task {
    if (server.worldContainer.freeSpace.toDouble().div(0x4000_0000) > 3) STABLE.withDiagnostics()
        ...
        WARN.create("- 3GB of free space is required for KrudWorld to function.")
    )
}

private val java by task {
    val version = KrudWorld.getJavaVersion()
