...
import dev.krud.world.util.interpolation.KrudWorldInterpolation.NoiseKey;
...
public class KrudWorldComplex implements DataProvider {
    private static final BlockData AIR = Material.AIR.createBlockData();
    private RNG rng;
    private double fluidHeight;
    private KrudWorldData data;
    private Map<KrudWorldInterpolator, Set<KrudWorldGenerator>> generators;
    private ProceduralStream<KrudWorldRegion> regionStream;
    ...
    private ProceduralStream<KrudWorldBiome> landBiomeStream;
    private ProceduralStream<KrudWorldBiome> caveBiomeStream;
    private ProceduralStream<KrudWorldBiome> seaBiomeStream;
    private ProceduralStream<KrudWorldBiome> shoreBiomeStream;
    private ProceduralStream<KrudWorldBiome> baseBiomeStream;
    private ProceduralStream<UUID> baseBiomeIDStream;
    private ProceduralStream<KrudWorldBiome> trueBiomeStream;
    ...
    private ProceduralStream<KrudWorldDecorator> terrainSurfaceDecoration;
    private ProceduralStream<KrudWorldDecorator> terrainCeilingDecoration;
    private ProceduralStream<KrudWorldDecorator> terrainCaveSurfaceDecoration;
    private ProceduralStream<KrudWorldDecorator> terrainCaveCeilingDecoration;
    private ProceduralStream<KrudWorldDecorator> seaSurfaceDecoration;
    private ProceduralStream<KrudWorldDecorator> seaFloorDecoration;
    private ProceduralStream<KrudWorldDecorator> shoreSurfaceDecoration;
    private ProceduralStream<BlockData> rockStream;
    private ProceduralStream<BlockData> fluidStream;
    private KrudWorldBiome focusBiome;
    private KrudWorldRegion focusRegion;

    public KrudWorldComplex(Engine engine) {
        this(engine, false);
    }

    public KrudWorldComplex(Engine engine, boolean simple) {
        int cacheSize = KrudWorldSettings.get().getPerformance().getNoiseCacheSize();
        KrudWorldBiome emptyBiome = new KrudWorldBiome();
        ...
        Map<InferredType, ProceduralStream<KrudWorldBiome>> inferredStreams = new HashMap<>();
        ...
        caveBiomeStream = regionStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getRegion().get(x, z))
                .convert((r)
        ...
        landBiomeStream = regionStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getRegion().get(x, z))
                .convert((r)
        ...
        seaBiomeStream = regionStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getRegion().get(x, z))
                .convert((r)
        ...
        shoreBiomeStream = regionStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getRegion().get(x, z))
                .convert((r)
        ...
        heightStream = ProceduralStream.of((x, z) -> {
            KrudWorldBiome b = focusBiome != null ? focusBiome : baseBiomeStream.get(x, z);
        ...
        roundedHeighteightStream = heightStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getHeight().get(x, z))
                .round().waste("Rounded Height Stream");
        slopeStream = heightStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getHeight().get(x, z))
        ...
        trueBiomeStream = focusBiome != null ? ProceduralStream.of((x, y) -> focusBiome, Interpolated.of(a -> 0D,
                ...
                .convertAware2D((h, x, z) ->
                        fixBiomeType(h, baseBiomeStream.get(x, z),
                                regionStream.contextInjecting((c, xx, zz) -> KrudWorldContext.getOr(engine).getChunkContext().getRegion().get(xx, zz)).get(x, z), x, z, fluidHeight))
                .cache2D("trueBiomeStream", engine, cacheSize).waste("True Biome Stream");
        trueBiomeDerivativeStream = trueBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getBiome().get(x, z))
                .convert(KrudWorldBiome::getDerivative).cache2D("trueBiomeDerivativeStream", engine, cacheSize).waste("True Biome Derivative Stream");
        heightFluidStream = heightStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getHeight().get(x, z))
        ...
        terrainSurfaceDecoration = trueBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getBiome().get(x, z))
                .convertAware2D((b, xx, zz) -> decorateFor(b, xx, zz, KrudWorldDecorationPart.NONE)).cache2D("terrainSurfaceDecoration", engine, cacheSize).waste("Surface Decoration Stream");
        terrainCeilingDecoration = trueBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getBiome().get(x, z))
                .convertAware2D((b, xx, zz) -> decorateFor(b, xx, zz, KrudWorldDecorationPart.CEILING)).cache2D("terrainCeilingDecoration", engine, cacheSize).waste("Ceiling Decoration Stream");
        terrainCaveSurfaceDecoration = caveBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getCave().get(x, z))
                .convertAware2D((b, xx, zz) -> decorateFor(b, xx, zz, KrudWorldDecorationPart.NONE)).cache2D("terrainCaveSurfaceDecoration", engine, cacheSize).waste("Cave Surface Stream");
        terrainCaveCeilingDecoration = caveBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getCave().get(x, z))
                .convertAware2D((b, xx, zz) -> decorateFor(b, xx, zz, KrudWorldDecorationPart.CEILING)).cache2D("terrainCaveCeilingDecoration", engine, cacheSize).waste("Cave Ceiling Stream");
        shoreSurfaceDecoration = trueBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getBiome().get(x, z))
                .convertAware2D((b, xx, zz) -> decorateFor(b, xx, zz, KrudWorldDecorationPart.SHORE_LINE)).cache2D("shoreSurfaceDecoration", engine, cacheSize).waste("Shore Surface Stream");
        seaSurfaceDecoration = trueBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getBiome().get(x, z))
                .convertAware2D((b, xx, zz) -> decorateFor(b, xx, zz, KrudWorldDecorationPart.SEA_SURFACE)).cache2D("seaSurfaceDecoration", engine, cacheSize).waste("Sea Surface Stream");
        seaFloorDecoration = trueBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getBiome().get(x, z))
                .convertAware2D((b, xx, zz) -> decorateFor(b, xx, zz, KrudWorldDecorationPart.SEA_FLOOR)).cache2D("seaFloorDecoration", engine, cacheSize).waste("Sea Floor Stream");
        baseBiomeIDStream = trueBiomeStream.contextInjecting((c, x, z) -> KrudWorldContext.getOr(engine).getChunkContext().getBiome().get(x, z))
    ...
    public ProceduralStream<KrudWorldBiome> getBiomeStream(InferredType type) {
        switch (type) {
            case CAVE:
    ...
    private KrudWorldRegion findRegion(KrudWorldBiome focus, Engine engine) {
        for (KrudWorldRegion i : engine.getDimension().getAllRegions(engine)) {
        ...
        KrudWorldRegion region = new KrudWorldRegion();
    ...
    private KrudWorldDecorator decorateFor(KrudWorldBiome b, double x, double z, KrudWorldDecorationPart part) {
        RNG rngc = new RNG(Cache.key(((int) x), ((int) z)));

        for (KrudWorldDecorator i : b.getDecorators()) {
    ...
    private KrudWorldBiome fixBiomeType(Double height, KrudWorldBiome biome, KrudWorldRegion region, Double x, Double z, double fluidHeight) {
    ...
    private double interpolateGenerators(Engine engine, KrudWorldInterpolator interpolator, Set<KrudWorldGenerator> generators, double x, double z, long seed) {
        if (generators.isEmpty()) {
            return 0;
        }

        HashMap<NoiseKey, KrudWorldBiome> cache = new HashMap<>(64);
        double hi = interpolator.interpolate(x, z, (xx, zz) -> {
            try {
                KrudWorldBiome bx = baseBiomeStream.get(xx, zz);
                cache.put(new NoiseKey(xx, zz), bx);
                double b = 0;

                for (KrudWorldGenerator gen : generators) {
                ...
                e.printStackTrace();
                KrudWorld.error("Failed to sample hi biome at " + xx + " " + zz + "...");
            }
        ...
        double lo = interpolator.interpolate(x, z, (xx, zz) -> {
            try {
                KrudWorldBiome bx = cache.get(new NoiseKey(xx, zz));
                if (bx == null) {
                ...
                KrudWorld.reportError(e);
                e.printStackTrace();
                KrudWorld.error("Failed to sample lo biome at " + xx + " " + zz + "...");
            }
    ...
    private double getInterpolatedHeight(Engine engine, double x, double z, long seed) {
        ...
        for (KrudWorldInterpolator i : generators.keySet()) {
    ...
    private KrudWorldBiome implode(KrudWorldBiome b, Double x, Double z, int max) {
        if (max < 0) {
            return b;
        }
        ...
        KList<KrudWorldBiome> chx = b.getRealChildren(this).copy();
        chx.add(b);
        KrudWorldBiome biome = childCell.fitRarity(chx, x, z);
