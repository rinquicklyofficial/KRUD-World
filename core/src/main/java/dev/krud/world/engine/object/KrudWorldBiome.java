...
public class KrudWorldBiome extends KrudWorldRegistrant implements IRare {
    private static final BlockData BARRIER = Material.BARRIER.createBlockData();
    private final transient AtomicCache<KMap<String, KrudWorldBiomeGeneratorLink>> genCache = new AtomicCache<>();
    ...
    private final transient AtomicCache<KrudWorldBiome> realCarveBiome = new AtomicCache<>();
    private final transient AtomicCache<KList<KrudWorldBiome>> realChildren = new AtomicCache<>();
    ...
    @ArrayType(min = 1, type = KrudWorldBiomeCustom.class)
    @Desc("If the biome type custom is defined, specify this")
    private KList<KrudWorldBiomeCustom> customDerivitives;
    @Desc("Spawn Entities in this area over time. KrudWorld will continually replenish these mobs just like vanilla does.")
    ...
    private KrudWorldGeneratorStyle biomeStyle = NoiseStyle.SIMPLEX.style();
    ...
    private KrudWorldGeneratorStyle childStyle = NoiseStyle.CELLULAR_IRIS_DOUBLE.style();
    @RegistryListResource(KrudWorldBiome.class)
    @ArrayType(min = 1, type = String.class)
    @Desc("List any biome names (file names without.json) here as children. Portions of this biome can sometimes morph into their children. KrudWorld supports cyclic relationships such as A > B > A > B. KrudWorld will stop checking 9 biomes down the tree.")
    ...
    private KrudWorldBiomePaletteLayer slab = new KrudWorldBiomePaletteLayer().zero();
    @Desc("The default wall if iris decides to place a wall higher than 2 blocks (steep hills or possibly cliffs)")
    private KrudWorldBiomePaletteLayer wall = new KrudWorldBiomePaletteLayer().zero();
    @Required
    @ArrayType(type = KrudWorldBiomePaletteLayer.class)
    @Desc("This defines the layers of materials in this biome. Each layer has a palette and min/max height and some other properties. Usually a grassy/sandy layer then a dirt layer then a stone layer. KrudWorld will fill in the remaining blocks below your layers with stone.")
    private KList<KrudWorldBiomePaletteLayer> layers = new KList<KrudWorldBiomePaletteLayer>().qadd(new KrudWorldBiomePaletteLayer());
    @Required
    @ArrayType(type = KrudWorldBiomePaletteLayer.class)
    @Desc("This defines the layers of materials in this biome. Each layer has a palette and min/max height and some other properties. Usually a grassy/sandy layer then a dirt layer then a stone layer. KrudWorld will fill in the remaining blocks below your layers with stone.")
    private KList<KrudWorldBiomePaletteLayer> caveCeilingLayers = new KList<KrudWorldBiomePaletteLayer>().qadd(new KrudWorldBiomePaletteLayer());
    @ArrayType(type = KrudWorldBiomePaletteLayer.class)
    @Desc("This defines the layers of materials in this biome. Each layer has a palette and min/max height and some other properties. Usually a grassy/sandy layer then a dirt layer then a stone layer. KrudWorld will fill in the remaining blocks below your layers with stone.")
    private KList<KrudWorldBiomePaletteLayer> seaLayers = new KList<>();
    @ArrayType(min = 1, type = KrudWorldDecorator.class)
    @Desc("Decorators are used for things like tall grass, bisected flowers, and even kelp or cactus (random heights)")
    private KList<KrudWorldDecorator> decorators = new KList<>();
    ...
    @Required
    @ArrayType(min = 1, type = KrudWorldBiomeGeneratorLink.class)
    ...
    private KList<KrudWorldBiomeGeneratorLink> generators = new KList<KrudWorldBiomeGeneratorLink>().qadd(new KrudWorldBiomeGeneratorLink());
    ...
    public double getGenLinkMax(String loadKey, Engine engine) {
        Integer v = genCacheMax.aquire(() ->
        {
            ...
            for (KrudWorldBiomeGeneratorLink i : getGenerators()) {
    ...
    public KrudWorldBiomeGeneratorLink getGenLink(String loadKey) {
        return genCache.aquire(() ->
        {
            KMap<String, KrudWorldBiomeGeneratorLink> l = new KMap<>();
    ...
    public KrudWorldBiome getRealCarvingBiome(KrudWorldData data) {
        return realCarveBiome.aquire(() ->
        {
            KrudWorldBiome biome = data.getBiomeLoader().load(getCarvingBiome());
    ...
    public double getHeight(Engine xg, double x, double z, long seed) {
        ...
        for (KrudWorldBiomeGeneratorLink i : generators) {
    ...
    public KList<BlockData> generateLayers(KrudWorldDimension dim, double wx, double wz, RNG random, int maxDepth, int height, KrudWorldData rdata, KrudWorldComplex complex) {
        if (isLockLayers()) {
        ...
        for (int i = 0; i < layers.size(); i++) {
            ...
            KrudWorldSlopeClip sc = getLayers().get(i).getSlopeCondition();
    ...
    public KList<BlockData> generateCeilingLayers(KrudWorldDimension dim, double wx, double wz, RNG random, int maxDepth, int height, KrudWorldData rdata, KrudWorldComplex complex) {
        KList<BlockData> data = new KList<>();
    ...
    public KList<BlockData> generateLockedLayers(double wx, double wz, RNG random, int maxDepthf, int height, KrudWorldData rdata, KrudWorldComplex complex) {
        KList<BlockData> data = new KList<>();
    ...
    public int getMaxWithObjectHeight(KrudWorldData data, Engine engine) {
        return maxWithObjectHeight.aquire(() ->
        {
            ...
            for (KrudWorldObjectPlacement i : getObjects()) {
                for (KrudWorldObject j : data.getObjectLoader().loadAll(i.getPlace())) {
    ...
    public KList<BlockData> generateSeaLayers(double wx, double wz, RNG random, int maxDepth, KrudWorldData rdata) {
        KList<BlockData> data = new KList<>();
    ...
    public KList<CNG> getLayerHeightGenerators(RNG rng, KrudWorldData rdata) {
        return layerHeightGenerators.aquire(() ->
        {
            ...
            for (KrudWorldBiomePaletteLayer i : getLayers()) {
    ...
    public KList<CNG> getLayerSeaHeightGenerators(RNG rng, KrudWorldData data) {
        return layerSeaHeightGenerators.aquire(() ->
        {
            ...
            for (KrudWorldBiomePaletteLayer i : getSeaLayers()) {
    ...
    public KrudWorldBiomeCustom getCustomBiome(RNG rng, double x, double y, double z) {
    ...
    public KList<KrudWorldBiome> getRealChildren(DataProvider g) {
        return realChildren.aquire(() ->
        {
            KList<KrudWorldBiome> realChildren = new KList<>();
    ...
    public KList<String> getAllChildren(DataProvider g, int limit) {
        ...
        if (limit > 0) {
            for (String i : getChildren()) {
                KrudWorldBiome b = g.getData().getBiomeLoader().load(i);
    ...
    public BlockData getSurfaceBlock(int x, int z, RNG rng, KrudWorldData idm) {
        ...
        switch (type) {
                ...
                return this.cacheColor.aquire(() -> {
                    if (this.color == null) {
                        ...
                        if (this.getVanillaDerivative() == null) {
                            KrudWorld.warn("No vanilla biome found for " + getName());
                    ...
                    try {
                        ...
                        KrudWorld.warn("Could not parse color \"" + this.color + "\" for biome " + getName());
            ...
            case DECORATOR_LOAD -> {
                return cacheColorDecoratorLoad.aquire(() -> {
                    ...
                    for (KrudWorldDecorator i : getDecorators()) {
