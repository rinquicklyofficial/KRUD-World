...
import dev.krud.world.engine.framework.PlacedObject;
import dev.krud.world.engine.framework.placer.HeightmapObjectPlacer;
...
import dev.krud.world.util.data.VectorMap;
...
import dev.krud.world.util.math.*;
...
public class KrudWorldObject extends KrudWorldRegistrant {
    ...
    public KrudWorldObject(int w, int h, int d) {
    ...
    public static AxisAlignedBB getAABBFor(BlockVector size) {
        ...
        return new AxisAlignedBB(new KrudWorldPosition(new BlockVector(0, 0, 0).subtract(center).toBlockVector()),
                new KrudWorldPosition(new BlockVector(size.getX() - 1, size.getY() - 1, size.getZ() - 1).subtract(center).toBlockVector()));
    }
    ...
    public void ensureSmartBored(boolean debug) {
        ...
        if (blocks.isEmpty()) {
            writeLock.unlock();
            KrudWorld.warn("Cannot Smart Bore " + getLoadKey() + " because it has 0 blocks in it.");
            smartBored = true;
            return;
        }
        ...
        smartBored = true;
        writeLock.unlock();
        KrudWorld.debug("Smart Bore: " + getLoadKey() + " in " + Form.duration(p.getMilliseconds(), 2) + " (" + Form.f(applied.get()) + ")");
    }

    public synchronized KrudWorldObject copy() {
        KrudWorldObject o = new KrudWorldObject(w, h, d);
    ...
    public void read(InputStream in) throws Throwable {
        ...
        this.h = din.readInt();
        this.d = din.readInt();
        if (!din.readUTF().equals("KrudWorld V2 IOB;")) {
    ...
    public void setUnsigned(int x, int y, int z, Block block, boolean legacy) {
        ...
        if (block == null) {
            ...
            if (state != null) {
                KrudWorld.debug("Saved State " + v);
                states.put(v, state);
            }
        }
    }

    public int place(int x, int z, IObjectPlacer placer, KrudWorldObjectPlacement config, RNG rng, KrudWorldData rdata) {
        return place(x, -1, z, placer, config, rng, rdata);
    }

    public int place(int x, int z, IObjectPlacer placer, KrudWorldObjectPlacement config, RNG rng, CarveResult c, KrudWorldData rdata) {
        return place(x, -1, z, placer, config, rng, null, c, rdata);
    }

    public int place(int x, int yv, int z, IObjectPlacer placer, KrudWorldObjectPlacement config, RNG rng, KrudWorldData rdata) {
        return place(x, yv, z, placer, config, rng, null, null, rdata);
    }

    public int place(Location loc, IObjectPlacer placer, KrudWorldObjectPlacement config, RNG rng, KrudWorldData rdata) {
        return place(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), placer, config, rng, rdata);
    }

    public int place(int x, int yv, int z, IObjectPlacer oplacer, KrudWorldObjectPlacement config, RNG rng, BiConsumer<BlockPosition, BlockData> listener, CarveResult c, KrudWorldData rdata) {
        IObjectPlacer placer = (config.getHeightmap() != null) ? new HeightmapObjectPlacer(oplacer.getEngine() == null ? KrudWorldContext.get().getEngine() : oplacer.getEngine(), rng, x, yv, z, config, oplacer) : oplacer;
...
//            }
            ...
            if (config.isRotateTowardsSlope()) {
                ...
                if (newRotation == 0) {
                    config.getRotation().setYAxis(new KrudWorldAxisRotationClamp(false, false, 0, 0, 90));
                    config.getRotation().setEnabled(config.getRotation().canRotateX() || config.getRotation().canRotateZ());
                } else {
                    config.getRotation().setYAxis(new KrudWorldAxisRotationClamp(true, false, newRotation, newRotation, 90));
        ...
        try {
            if (config.getMarkers().isNotEmpty() && placer.getEngine() != null) {
                ...
                for (KrudWorldObjectMarker j : config.getMarkers()) {
                    KrudWorldMarker marker = getLoader().getMarkerLoader().load(j.getMarker());
            ...
            for (var entry : blocks) {
                ...
                try {
                    ...
                    KrudWorld.reportError(e);
                    KrudWorld.warn("Failed to read block node " + g.getBlockX() + "," + g.getBlockY() + "," + g.getBlockZ() + " in object " + getLoadKey() + " (cme)");
                    d = AIR;
                }

                if (d == null) {
                    KrudWorld.warn("Failed to read block node " + g.getBlockX() + "," + g.getBlockY() + "," + g.getBlockZ() + " in object " + getLoadKey() + " (null)");
                    d = AIR;
                }
                ...
                for (KrudWorldObjectReplace j : config.getEdit()) {
                    if (rng.chance(j.getChance())) {
                        for (BlockData k : j.getFind(rdata)) {
                            if (j.isExact() ? k.matches(data) : k.getMaterial().equals(data.getMaterial())) {
                                ...
                                if (newData.getMaterial() == data.getMaterial() && !(newData instanceof KrudWorldCustomData || data instanceof KrudWorldCustomData))
                ...
                if (data instanceof KrudWorldCustomData || place) {
        ...
        if (stilting) {
            readLock.lock();
            KrudWorldStiltSettings settings = config.getStiltSettings();
            for (BlockVector g : blocks.keys()) {
                ...
                if (settings == null || settings.getPalette() == null) {
                    try {
                        ...
                        KrudWorld.reportError(e);
                        KrudWorld.warn("Failed to read block node " + g.getBlockX() + "," + g.getBlockY() + "," + g.getBlockZ() + " in object " + getLoadKey() + " (stilt cme)");
                        d = AIR;
                    }

                    if (d == null) {
                        KrudWorld.warn("Failed to read block node " + g.getBlockX() + "," + g.getBlockY() + "," + g.getBlockZ() + " in object " + getLoadKey() + " (stilt null)");
                        d = AIR;
                    }
                } else
    ...
    public KrudWorldObject rotateCopy(KrudWorldObjectRotation rt) {
        KrudWorldObject copy = copy();
        copy.rotate(rt, 0, 0, 0);
        return copy;
    }

    public void rotate(KrudWorldObjectRotation r, int spinx, int spiny, int spinz) {
        writeLock.lock();
        VectorMap<BlockData> d = new VectorMap<>();
    ...
    public void place(Location at) {
        ...
        for (var entry : blocks) {
            ...
            if (states.containsKey(i)) {
                KrudWorld.info(Objects.requireNonNull(states.get(i)).toString());
    ...
    public KrudWorldObject scaled(double scale, KrudWorldObjectPlacementScaleInterpolator interpolation) {
        ...
        KrudWorldPosition l1 = getAABB().max();
        KrudWorldPosition l2 = getAABB().min();
        ...
        KrudWorldObject oo = new KrudWorldObject((int) Math.ceil((w * scale) + (scale * 2)), (int) Math.ceil((h * scale) + (scale * 2)), (int) Math.ceil((d * scale) + (scale * 2)));
        ...
        for (int x = min.getBlockX(); x <= max.getBlockX(); x++) {
            for (int y = min.getBlockY(); y <= max.getBlockY(); y++) {
                for (int z = min.getBlockZ(); z <= max.getBlockZ(); z++) {
                    if (KrudWorldInterpolation.getTrilinear(x, y, z, rad, (xx, yy, zz) -> {
    ...
    public void tricubic(int rad) {
        ...
        for (int x = min.getBlockX(); x <= max.getBlockX(); x++) {
            for (int y = min.getBlockY(); y <= max.getBlockY(); y++) {
                for (int z = min.getBlockZ(); z <= max.getBlockZ(); z++) {
                    if (KrudWorldInterpolation.getTricubic(x, y, z, rad, (xx, yy, zz) -> {
                        BlockData data = v.get(new BlockVector((int) xx, (int) yy, (int) zz));
    ...
    public void trihermite(int rad, double tension, double bias) {
        ...
        for (int x = min.getBlockX(); x <= max.getBlockX(); x++) {
            for (int y = min.getBlockY(); y <= max.getBlockY(); y++) {
                for (int z = min.getBlockZ(); z <= max.getBlockZ(); z++) {
                    if (KrudWorldInterpolation.getTrihermite(x, y, z, rad, (xx, yy, zz) -> {
                        BlockData data = v.get(new BlockVector((int) xx, (int) yy, (int) zz));
