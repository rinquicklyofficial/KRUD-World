...
import dev.krud.world.util.noise.ExpressionNoise;
import dev.krud.world.util.noise.ImageNoise;
import dev.krud.world.util.noise.NoiseGenerator;
...
@Desc("A gen style")
@Data
public class KrudWorldGeneratorStyle {
    ...
    @RegistryListResource(KrudWorldExpression.class)
    private String expression = null;
    @Desc("Use an Image map instead of a generated value")
    private KrudWorldImageMap imageMap = null;
    ...
    private KrudWorldGeneratorStyle fracture = null;
    @MinNumber(0.01562)
    @MaxNumber(64)
    @Desc("The exponent")
    ...
    public KrudWorldGeneratorStyle(NoiseStyle s) {
        this.style = s;
    }

    public KrudWorldGeneratorStyle zoomed(double z) {
        this.zoom = z;
        return this;
    }

    public CNG createNoCache(RNG rng, KrudWorldData data) {
    ...
    public CNG createNoCache(RNG rng, KrudWorldData data, boolean actuallyCached) {
        String cacheKey = hash() + "";

        CNG cng = null;
        if (getExpression() != null) {
            KrudWorldExpression e = data.getExpressionLoader().load(getExpression());
            if (e != null) {
            ...
            if (result == null) KrudWorld.warn("Failed to create noise from script: " + getScript());
    ...
    public double warp(RNG rng, KrudWorldData data, double value, double... coords) {
        return create(rng, data).noise(coords) + value;
    }

    public CNG create(RNG rng, KrudWorldData data) {
