...
import dev.krud.world.util.noise.CNGFactory;
...
public enum NoiseStyle {
    ...
    @Desc("Billow Fractal KrudWorld Noise. Single octave.")
    FRACTAL_BILLOW_IRIS(rng -> CNG.signature(rng, NoiseType.FRACTAL_BILLOW_SIMPLEX)),

    @Desc("FBM Fractal KrudWorld Noise. Single octave.")
    ...
    @Desc("Fractal Cubic Noise With KrudWorld Swirls")
    ...
    @Desc("Cellular noise creates the same noise level for cells, changes noise level on cell borders. Cells are distorted using KrudWorld styled wispy noise.")
    ...
    @Desc("Vascular noise gets higher as the position nears a cell border. Cells are distorted using KrudWorld styled wispy noise.")
    ...
    public KrudWorldGeneratorStyle style() {
        return new KrudWorldGeneratorStyle(this);
    }
}
