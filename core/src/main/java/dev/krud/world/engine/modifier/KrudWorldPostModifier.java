...
import dev.krud.world.engine.object.KrudWorldSlopeClip;
...
public class KrudWorldPostModifier extends EngineAssignedModifier<BlockData> {
    private static final BlockData AIR = B.get("AIR");
    private static final BlockData WATER = B.get("WATER");
    private final RNG rng;

    public KrudWorldPostModifier(Engine engine) {
        super(engine, "Post");
    ...
    private void post(int currentPostX, int currentPostZ, Hunk<BlockData> currentData, int x, int z, ChunkContext context) {
        ...
        // Wall Patcher
        KrudWorldBiome biome = context.getBiome().get(currentPostX, currentPostZ);
        ...
        if (getDimension().isPostProcessingSlabs()) {
            ...
            //@done
            {
                KrudWorldSlopeClip sc = biome.getSlab().getSlopeCondition();
