...
import dev.krud.world.util.collection.GBiset;
import dev.krud.world.util.collection.KList;
import dev.krud.world.util.collection.KMap;
import dev.krud.world.util.data.Cuboid.CuboidDirection;
import dev.krud.world.util.math.DOP;
import dev.krud.world.util.math.VectorMath;
...
public enum KrudWorldDirection {
    @Desc("0, 1, 0")
    ...
    private static KMap<GBiset<KrudWorldDirection, KrudWorldDirection>, DOP> permute = null;
    ...
    KrudWorldDirection(int x, int y, int z, CuboidDirection f) {
        this.x = x;
        this.y = y;
        this.z = z;
        this.f = f;
    ...
    public static KrudWorldDirection getDirection(BlockFace f) {
        return switch (f) {
    ...
    public static KrudWorldDirection fromJigsawBlock(String direction) {
        for (KrudWorldDirection i : KrudWorldDirection.values()) {
    ...
    public static KrudWorldDirection getDirection(Jigsaw.Orientation orientation) {
    ...
    public static KrudWorldDirection closest(Vector v) {
        double m = Double.MAX_VALUE;
        KrudWorldDirection s = null;

        for (KrudWorldDirection i : values()) {
            Vector x = i.toVector();
    ...
    public static KrudWorldDirection closest(Vector v, KrudWorldDirection... d) {
        double m = Double.MAX_VALUE;
        KrudWorldDirection s = null;

        for (KrudWorldDirection i : d) {
            Vector x = i.toVector();
            double g = x.distance(v);
    ...
    public static KrudWorldDirection closest(Vector v, KList<KrudWorldDirection> d) {
        double m = Double.MAX_VALUE;
        KrudWorldDirection s = null;
    ...
    public static KList<KrudWorldDirection> news() {
        return new KList<KrudWorldDirection>().add(NORTH_NEGATIVE_Z, EAST_POSITIVE_X, WEST_NEGATIVE_X, SOUTH_POSITIVE_Z);
    }

    public static KrudWorldDirection getDirection(Vector v) {
        Vector k = VectorMath.triNormalize(v.clone().normalize());

        for (KrudWorldDirection i : udnews()) {
            if (i.x == k.getBlockX() && i.y == k.getBlockY() && i.z == k.getBlockZ()) {
                return i;
            }
        }

        return KrudWorldDirection.NORTH_NEGATIVE_Z;
    }

    public static KList<KrudWorldDirection> udnews() {
        return new KList<KrudWorldDirection>().add(UP_POSITIVE_Y, DOWN_NEGATIVE_Y, NORTH_NEGATIVE_Z, EAST_POSITIVE_X, WEST_NEGATIVE_X, SOUTH_POSITIVE_Z);
    }
     ...
     * 0-5
     */
    public static KrudWorldDirection fromByte(byte b) {
        if (b > 5 || b < 0) {
            return null;
        }
    ...
    public static void calculatePermutations() {
        ...
        for (KrudWorldDirection i : udnews()) {
            for (KrudWorldDirection j : udnews()) {
                GBiset<KrudWorldDirection, KrudWorldDirection> b = new GBiset<>(i, j);
    ...
    public boolean isCrooked(KrudWorldDirection to) {
    ...
    public Vector angle(Vector initial, KrudWorldDirection d) {
        calculatePermutations();

        for (Map.Entry<GBiset<KrudWorldDirection, KrudWorldDirection>, DOP> entry : permute.entrySet()) {
            GBiset<KrudWorldDirection, KrudWorldDirection> i = entry.getKey();
    ...
    public KrudWorldDirection reverse() {
        switch (this) {
