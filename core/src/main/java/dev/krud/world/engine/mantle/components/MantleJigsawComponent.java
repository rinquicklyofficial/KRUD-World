...
public class MantleJigsawComponent extends KrudWorldMantleComponent {
    ...
    @ChunkCoordinates
    private void generateJigsaw(MantleWriter writer, int x, int z, KrudWorldBiome biome, KrudWorldRegion region) {
            ...
            if (poss != null) {
                for (Position2 pos : poss) {
                    if (x == pos.getX() >> 4 && z == pos.getZ() >> 4) {
                        KrudWorldJigsawStructure structure = getData().getJigsawStructureLoader().load(getDimension().getStronghold());
                        place(writer, pos.toKrudWorld(), structure, new RNG(seed), true);
                        return;
                    }
                }
            }
    ...
    @ChunkCoordinates
    private boolean placeStructures(MantleWriter writer, long seed, int x, int z, KList<KrudWorldJigsawStructurePlacement> structures,
            KSet<Position2> cachedRegions, KMap<String, KSet<Position2>> cache, KMap<Position2, Double> distanceCache) {
        KrudWorldJigsawStructurePlacement i = pick(structures, seed, x, z);
        try {
            if (i == null || checkMinDistances(i.collectMinDistances(), x, z, cachedRegions, cache, distanceCache))
                return false;
        } catch (Throwable ignored) {}
        RNG rng = new RNG(seed);
        KrudWorldPosition position = new KrudWorldPosition((x << 4) + rng.nextInt(15), 0, (z << 4) + rng.nextInt(15));
        KrudWorldJigsawStructure structure = getData().getJigsawStructureLoader().load(i.getStructure());
    ...
    @ChunkCoordinates
    public KrudWorldJigsawStructure guess(int x, int z) {
        ...
        KrudWorldJigsawStructurePlacement i = pick(biome.getJigsawStructures(), seed, x, z);
    ...
    @Nullable
    @ChunkCoordinates
    private KrudWorldJigsawStructurePlacement pick(List<KrudWorldJigsawStructurePlacement> structures, long seed, int x, int z) {
    ...
    @BlockCoordinates
    private boolean place(MantleWriter writer, KrudWorldPosition position, KrudWorldJigsawStructure structure, RNG rng, boolean forcePlace) {
