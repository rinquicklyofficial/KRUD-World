...
public class GlobalCacheSVC implements KrudWorldService {
    ...
    public void onEnable() {
        ...
        trimmer.start();
        lastState = !KrudWorldSettings.get().getWorld().isGlobalPregenCache();
        if (lastState) return;
    ...
    private void createCache(World world) {
        if (!KrudWorldToolbelt.isKrudWorldWorld(world)) return;
        globalCache.computeIfAbsent(world.getName(), GlobalCacheSVC::createDefault);
    }

    private boolean isDisabled() {
        boolean conf = KrudWorldSettings.get().getWorld().isGlobalPregenCache();
        if (lastState != conf)
            return lastState;
