...
import dev.krud.world.util.plugin.chunk.TicketHolder;
import dev.krud.world.util.profile.LoadBalancer;
...
public class ChunkUpdater {
    ...
    private final int maxConcurrency = KrudWorldSettings.get().getUpdater().getMaxConcurrency();
    private final int coreLimit = (int) Math.max(Runtime.getRuntime().availableProcessors() * KrudWorldSettings.get().getUpdater().getThreadMultiplier(), 1);
    private final Semaphore semaphore = new Semaphore(maxConcurrency);
    private final LoadBalancer loadBalancer = new LoadBalancer(semaphore, maxConcurrency, KrudWorldSettings.get().getUpdater().emptyMsRange);
    private final AtomicLong startTime = new AtomicLong();
    private final Dimensions dimensions;
    private final PregenTask task;
    private final ExecutorService chunkExecutor = KrudWorldSettings.get().getUpdater().isNativeThreads() ? Executors.newFixedThreadPool(coreLimit) : Executors.newVirtualThreadPerTaskExecutor();
    ...
    public ChunkUpdater(World world) {
        this.engine = KrudWorldToolbelt.access(world).getEngine();
        this.world = world;
        this.holder = KrudWorld.tickets.getHolder(world);
        ...
        try {
            ...
            scheduler.scheduleAtFixedRate(() -> {
                try {
                    if (!paused.get()) {
                        ...
                        if (!cancelled.get()) {
                            KrudWorld.info("Updated: " + Form.f(processed) + " of " + Form.f(totalMaxChunks.get()) + " (%.0f%%) " + Form.f(chunksPerSecond.getAverage()) + "/s, ETA: " + Form.duration(eta,
                                    2), percentage);
                        }
                    }
                } catch (Exception e) {
            ...
            scheduler.scheduleAtFixedRate(() -> {
                ...
                if (serverEmpty.getAndSet(empty) == empty)
                    return;
                loadBalancer.setRange(empty ? KrudWorldSettings.get().getUpdater().emptyMsRange : KrudWorldSettings.get().getUpdater().defaultMsRange);
            }, 0, 10, TimeUnit.SECONDS);

            var t = new Thread(() -> {
                run();
                close();
            }, "KrudWorld Chunk Updater - " + world.getName());
            t.setPriority(Thread.MAX_PRIORITY);
            t.start();

            KrudWorld.service(PreservationSVC.class).register(t);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void close() {
        ...
        if (cancelled.get()) {
            KrudWorld.info("Updated: " + Form.f(chunksUpdated.get()) + " Chunks");
            KrudWorld.info("Irritated: " + Form.f(chunksProcessed.get()) + " of " + Form.f(totalMaxChunks.get()));
            KrudWorld.info("Stopped updater.");
        } else {
            KrudWorld.info("Processed: " + Form.f(chunksProcessed.get()) + " Chunks");
            KrudWorld.info("Finished Updating: " + Form.f(chunksUpdated.get()) + " Chunks");
        }
    }
    ...
    private boolean loadChunksIfGenerated(int x, int z) {
        ...
        try {
            ...
            KrudWorld.info("Interrupted while waiting for chunks to load");
        }
