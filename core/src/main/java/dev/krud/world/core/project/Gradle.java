...
public class Gradle {
    ...
    public static synchronized void wrapper(File projectDir) {
        try {
            ...
            KrudWorld.error("Failed to install gradle wrapper!");
            e.printStackTrace();
    ...
    public static void runGradle(File projectDir, String... args) throws IOException, InterruptedException {
        ...
        if (code == 0) {
            lines.forEach(KrudWorld::debug);
            return;
        }
        lines.forEach(KrudWorld::error);
        throw new RuntimeException("Gradle exited with code " + code);
    }

    private static synchronized File downloadGradle(boolean force) {
        var folder = KrudWorld.instance.getDataFolder("cache", HASH.substring(0, 2), HASH);
        if (force) {
            IO.delete(folder);
            folder.mkdirs();
        }
