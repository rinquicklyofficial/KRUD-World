...
package dev.krud.world.core.gui;

import dev.krud.world.KrudWorld;
import dev.krud.world.core.gui.components.KrudWorldRenderer;
...
import static dev.krud.world.util.data.registry.Attributes.MAX_HEALTH;
...
public class VisionGUI extends JPanel implements MouseWheelListener, KeyListener, MouseMotionListener, MouseInputListener {
    ...
    private KrudWorldRenderer renderer;
    ...
    private final ExecutorService e = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors(), r -> {
        ...
        t.setName("KrudWorld HD Renderer " + tid);
        t.setPriority(Thread.MIN_PRIORITY);
        t.setUncaughtExceptionHandler((et, e) ->
        {
            KrudWorld.info("Exception encountered in " + et.getName());
            e.printStackTrace();
        });
    ...
    private final ExecutorService eh = Executors.newFixedThreadPool(ltc, r -> {
        tid++;
        Thread t = new Thread(r);
        t.setName("KrudWorld Renderer " + tid);
    ...
    private static void createAndShowGUI(Engine r, int s, KrudWorldWorld world) {
        JFrame frame = new JFrame("Vision");
        VisionGUI nv = new VisionGUI(frame);
        nv.world = world;
        nv.engine = r;
        nv.renderer = new KrudWorldRenderer(r);
        frame.add(nv);
        frame.setSize(1440, 820);
        frame.setVisible(true);
        File file = KrudWorld.getCached("KrudWorld Icon", "https://raw.githubusercontent.com/VolmitSoftware/KrudWorld/master/icon.png");
    ...
    public boolean updateEngine() {
        if (engine.isClosed()) {
            if (world.hasRealWorld()) {
                try {
                    engine = KrudWorldToolbelt.access(world.realWorld()).getEngine();
    ...
    private void renderHoverOverlay(Graphics2D g, boolean detailed) {
        KrudWorldBiome biome = engine.getComplex().getTrueBiomeStream().get(getWorldX(hx), getWorldZ(hz));
        KrudWorldRegion region = engine.getComplex().getRegionStream().get(getWorldX(hx), getWorldZ(hz));
        KList<String> l = new KList<>();
    ...
    private void open() {
        KrudWorldComplex complex = engine.getComplex();
        File r = null;
        switch (currentType) {
