...
import dev.krud.world.core.gui.NoiseExplorerGUI;
import dev.krud.world.core.gui.VisionGUI;
import dev.krud.world.core.loader.KrudWorldData;
import dev.krud.world.core.project.KrudWorldProject;
import dev.krud.world.core.service.ConversionSVC;
...
import dev.krud.world.util.parallel.SyncExecutor;
...
import dev.krud.world.util.scheduling.jobs.ParallelRadiusJob;
...
public class CommandStudio implements DecreeExecutor {
    ...
    public void download(
        ...
        new CommandKrudWorld().download(pack, branch, overwrite);
    }
    ...
    public void open(
        ...
        KrudWorld.service(StudioSVC.class).open(sender(), seed, dimension.getLoadKey());
    }
    ...
    public void vscode(
        ...
        KrudWorld.service(StudioSVC.class).openVSCode(sender(), dimension.getLoadKey());
    }
    ...
    public void create(
            ...
            KrudWorldDimension template) {
        if (template != null) {
            KrudWorld.service(StudioSVC.class).create(sender(), name, template.getLoadKey());
        } else {
            KrudWorld.service(StudioSVC.class).create(sender(), name);
        }
    }
    ...
    public void regen(
        ...
        if (!KrudWorldToolbelt.isKrudWorldWorld(world)) {
            sender().sendMessage(C.RED + "You must be in an KrudWorld World to use regen!");
        }
        ...
        J.a(() -> {
            PlatformChunkGenerator plat = KrudWorldToolbelt.access(world);
    ...
    public void convert() {
        KrudWorld.service(ConversionSVC.class).check(sender());
        //KrudWorldConverter.convertSchematics(sender());
    }

    @Decree(description = "Execute a script", aliases = "run", origin = DecreeOrigin.PLAYER)
    public void execute(
            @Param(description = "The script to run")
            KrudWorldScript script
    ) {
    ...
    public void charge() {
        if (!KrudWorldToolbelt.isKrudWorldWorld(world())) {
            sender().sendMessage(C.RED + "You must be in an KrudWorld world to charge spawners!");
            return;
        }
    ...
    public void hotload() {
        ...
        KrudWorld.service(StudioSVC.class).getActiveProject().getActiveProvider().getEngine().hotload();
    ...
    public void loot(
        ...
        KList<KrudWorldLootTable> tables = engine().getLootTables(RNG.r, player().getLocation().getBlock());
        ...
        ta.set(Bukkit.getScheduler().scheduleSyncRepeatingTask(KrudWorld.instance, () ->
        {
    ...
    public void regions(@Param(description = "The radius in chunks", defaultValue = "500") int radius) {
        var engine = engine();
        if (engine == null) {
            sender().sendMessage(C.RED + "Only works in an KrudWorld world!");
            return;
        }
        var sender = sender();
        var player = player();
    ...
    public void map(
        ...
        if (!KrudWorldToolbelt.isKrudWorldWorld(world)) {
            sender().sendMessage(C.RED + "You need to be in or specify an KrudWorld-generated world!");
            return;
        }

        VisionGUI.launch(KrudWorldToolbelt.access(world).getEngine(), 0);
    ...
    public void pkg(
        ...
        KrudWorld.service(StudioSVC.class).compilePackage(sender(), dimension.getLoadKey(), obfuscate, minify);
    }
    ...
    public void profile(
        ...
        File report = KrudWorld.instance.getDataFile("profile.txt");
        KrudWorldProject project = new KrudWorldProject(pack);
        KrudWorldData data = KrudWorldData.get(pack);
        ...
        for (InterpolationMethod i : InterpolationMethod.values()) {
            KrudWorldInterpolator in = new KrudWorldInterpolator();
            in.setFunction(i);
        ...
        for (String i : data.getGeneratorLoader().getPossibleKeys()) {
            ...
            KrudWorldGenerator g = data.getGeneratorLoader().load(i);
            KList<KrudWorldNoiseGenerator> composites = g.getAllComposites();
            double score = 0;
            int m = 0;
            for (KrudWorldNoiseGenerator j : composites) {
                m++;
        ...
        for (String i : data.getBiomeLoader().getPossibleKeys()) {
            KList<String> vv = new KList<>();
            KrudWorldBiome b = data.getBiomeLoader().load(i);
            double score = 0;

            int m = 0;
            for (KrudWorldBiomePaletteLayer j : b.getLayers()) {
                m++;
                score += styleTimings.get(j.getStyle().getStyle());
        ...
        for (String i : data.getRegionLoader().getPossibleKeys()) {
            KrudWorldRegion b = data.getRegionLoader().load(i);
            double score = 0;
    ...
    @Decree(description = "Spawn an KrudWorld entity", aliases = "summon", origin = DecreeOrigin.PLAYER)
    public void spawn(
            @Param(description = "The entity to spawn")
            KrudWorldEntity entity,
            @Param(description = "The location to spawn the entity at", contextual = true)
            Vector location
    ) {
        if (!KrudWorldToolbelt.isKrudWorldWorld(player().getWorld())) {
            sender().sendMessage(C.RED + "You have to be in an KrudWorld world to spawn entities properly. Trying to spawn the best we can do.");
        }
    ...
    public void tpstudio() {
        ...
        if (KrudWorldToolbelt.isKrudWorldWorld(world()) && engine().isStudio()) {
        ...
        var player = player();
        PaperLib.teleportAsync(player(), KrudWorld.service(StudioSVC.class)
                .getActiveProject()
                .getActiveProvider()
                .getTarget()
                .getWorld()
    ...
    public void update(
        ...
        if (new KrudWorldProject(dimension.getLoader().getDataFolder()).updateWorkspace()) {
    ...
    public void objects() {
        ...
        try {
            File ff = KrudWorld.instance.getDataFile("reports/" + M.ms() + ".txt");
            PrintWriter pw = new PrintWriter(ff);
            pw.println("=== KrudWorld Chunk Report ===");
            pw.println("== General Info ==");
            pw.println("KrudWorld Version: " + KrudWorld.instance.getDescription().getVersion());
            ...
            for (Chunk i : chunks) {
                for (int j = 0; j < 16; j += 3) {
                    ...
                    for (int k = 0; k < 16; k += 3) {
                        ...
                        assert engine() != null;
                        KrudWorldBiome bb = engine().getSurfaceBiome((i.getX() * 16) + j, (i.getZ() * 16) + k);
                        KrudWorldBiome bxf = engine().getCaveBiome((i.getX() * 16) + j, (i.getZ() * 16) + k);
    ...
    private void exportObjects(KrudWorldBiome bb, PrintWriter pw, Engine g, KMap<String, KMap<String, KList<String>>> objects) {
        String n1 = bb.getName() + " [" + Form.capitalize(bb.getInferredType().name().toLowerCase()) + "] " + " (" + bb.getLoadFile().getName() + ")";
        int m = 0;
        KSet<String> stop = new KSet<>();
        for (KrudWorldObjectPlacement f : bb.getObjects()) {
            m++;
            ...
            for (String i : f.getPlace()) {
                ...
                try {
                    ...
                    BlockVector sz = KrudWorldObject.sampleSize(ff);
    ...
    private boolean noGUI() {
        if (!KrudWorldSettings.get().getGui().isUseServerLaunchedGuis()) {
