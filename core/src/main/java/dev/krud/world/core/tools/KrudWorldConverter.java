...
import dev.krud.world.util.nbt.tag.*;
...
public class KrudWorldConverter {
    public static void convertSchematics(VolmitSender sender) {
        File folder = KrudWorld.instance.getDataFolder("convert");
        ...
        executorService.submit(() -> {
            for (File schem : fileList) {
                try {
                    ...
                    KrudWorldObject object;
                    boolean largeObject = false;
                    NamedTag tag;
                    try {
                        tag = NBTUtil.read(schem);
                    } catch (IOException e) {
                        KrudWorld.info(C.RED + "Failed to read: " + schem.getName());
                    ...
                    if (mv > 2_000_000) {
                        largeObject = true;
                        KrudWorld.info(C.GRAY + "Converting.. " + schem.getName() + " -> " + schem.getName().replace(".schem", ".iob"));
                        KrudWorld.info(C.GRAY + "- It may take a while");
                        if (sender.isPlayer()) {
                            i = J.ar(() -> {
                    ...
                    object = new KrudWorldObject(objW, objH, objD);
                    ...
                    try {
                        ...
                        if (largeObject) {
                            KrudWorld.info(C.GRAY + "Converted " + schem.getName() + " -> " + schem.getName().replace(".schem", ".iob") + " in " + Form.duration(p.getMillis()));
                        } else {
                            KrudWorld.info(C.GRAY + "Converted " + schem.getName() + " -> " + schem.getName().replace(".schem", ".iob"));
                        }
