...
import dev.krud.world.util.stream.utility.CachedStream2D;
import dev.krud.world.util.stream.utility.CachedStream3D;
...
public class KrudWorldEngineSVC implements KrudWorldService {
    ...
    public void onEnable() {
        var settings = KrudWorldSettings.get().getPerformance();
    ...
    public void engineStatus(VolmitSender sender) {
        ...
        for (var cache : KrudWorld.service(PreservationSVC.class).getCaches()) {
        ...
        sender.sendMessage(C.DARK_PURPLE + "- KrudWorld Worlds: " + C.LIGHT_PURPLE + totalWorlds.get());
    ...
    private void add(World world) {
        var access = KrudWorldToolbelt.access(world);
            ...
            if (trimmer == null || trimmer.isDone() || trimmer.isCancelled()) {
                trimmer = service.scheduleAtFixedRate(() -> {
                    ...
                    try {
                        ...
                        KrudWorld.error("EngineSVC: Failed to trim for " + name);
                        e.printStackTrace();
                    }
            ...
            if (unloader == null || unloader.isDone() || unloader.isCancelled()) {
                unloader = service.scheduleAtFixedRate(() -> {
                    ...
                    try {
                        ...
                        int count = engine.getMantle().unloadTectonicPlate(KrudWorldSettings.get().getPerformance().getEngineSVC().forceMulticoreWrite ? 0 : tectonicLimit());
                        if (count > 0) {
                            KrudWorld.debug(C.GOLD + "Unloaded " + C.YELLOW + count + " TectonicPlates in " + C.RED + Form.duration(System.currentTimeMillis() - unloadStart, 2));
                        }
                    } catch (Throwable e) {
                        KrudWorld.reportError(e);
                        KrudWorld.error("EngineSVC: Failed to unload for " + name);
                        e.printStackTrace();
                    }
