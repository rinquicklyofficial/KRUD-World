...
public class HMCLeavesDataProvider extends ExternalDataProvider {
	...
	public void init() {
		try {
			...
			KrudWorld.error("Failed to initialize HMCLeavesDataProvider: " + e.getMessage());
		}
	}
	...
	public BlockData getBlockData(@NotNull Identifier blockId, @NotNull KMap<String, String> state) throws MissingResourceException {
		...
		if (KrudWorldSettings.get().getGenerator().preventLeafDecay && blockData instanceof Leaves leaves)
			leaves.setPersistent(true);
		return KrudWorldCustomData.of(blockData, ExternalDataSVC.buildState(blockId, state));
	}
	...
	public void processUpdate(@NotNull Engine engine, @NotNull Block block, @NotNull Identifier blockId) {
		...
		if (result == null || !result)
			KrudWorld.warn("Failed to set custom block! " + blockId.key() + " " + block.getX() + " " + block.getY() + " " + block.getZ());
		else if (KrudWorldSettings.get().getGenerator().preventLeafDecay) {
