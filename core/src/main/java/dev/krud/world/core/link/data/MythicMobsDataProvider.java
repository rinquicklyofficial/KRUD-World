...
public class MythicMobsDataProvider extends ExternalDataProvider {
    ...
    public void on(MythicConditionLoadEvent event) {
        switch (event.getConditionName()) {
            case "irisbiome" -> event.register(new KrudWorldBiomeCondition(event.getConditionName(), event.getConfig()));
            case "irisregion" -> event.register(new KrudWorldRegionCondition(event.getConditionName(), event.getConfig()));
        }
    }

    @MythicCondition(author = "CrazyDev22", name = "irisbiome", description = "Tests if the target is within the given list of biomes")
    public static class KrudWorldBiomeCondition extends SkillCondition implements ILocationCondition {
        ...
        public KrudWorldBiomeCondition(String line, MythicLineConfig mlc) {
            super(line);
        ...
        public boolean check(AbstractLocation target) {
            var access = KrudWorldToolbelt.access(((BukkitWorld) target.getWorld()).getBukkitWorld());
    ...
    public static class KrudWorldRegionCondition extends SkillCondition implements ILocationCondition {
        ...
        public KrudWorldRegionCondition(String line, MythicLineConfig mlc) {
            super(line);
