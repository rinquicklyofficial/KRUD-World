...
import dev.krud.world.util.nbt.io.NBTDeserializer;
import dev.krud.world.util.nbt.io.NBTSerializer;
...
import static dev.krud.world.util.nbt.mca.LoadFlags.*;
...
public class Chunk {
    ...
    private int nativeKrudWorldVersion;
    ...
    public static void injectKrudWorldData(Chunk c) {
        World mainWorld = getServer().getWorlds().get(0);
        c.data.put("KrudWorld", nativeKrudWorldVersion());
    }

    private static CompoundTag defaultLevel() {
        CompoundTag level = new CompoundTag();
        level.putString("Status", "full");
        level.putString("Generator", "KrudWorld Headless " + KrudWorld.instance.getDescription().getVersion());
        return level;
    }

    private static CompoundTag nativeKrudWorldVersion() {
        CompoundTag level = new CompoundTag();
        level.putString("Generator", "KrudWorld " + KrudWorld.instance.getDescription().getVersion());
        return level;
    }
