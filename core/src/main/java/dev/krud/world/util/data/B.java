...
public class B {
    ...
    public static Material getMaterialOrNull(String bdx) {
        try {
            ...
            if (clw.flip()) {
                KrudWorld.warn("Unknown Material: " + bdx);
            }
            return null;
        }
    }
    ...
    public static BlockData getOrNull(String bdxf, boolean warn) {
        try {
            ...
            if (bdx == null && warn) {
                if (clw.flip()) {
                    KrudWorld.warn("Unknown Block Data '" + bd + "'");
                }
                return AIR;
            }
            ...
            if (clw.flip()) {
                KrudWorld.warn("Unknown Block Data '" + bdxf + "'");
            }
        }
    ...
    public static BlockData get(String bdxf) {
        if (bdxf.contains(":")) {
            if (bdxf.startsWith("minecraft:")) {
                return KrudWorld.compat.getBlock(bdxf);
            } else {
    ...
    private static synchronized BlockData createBlockData(String s, boolean warn) {
        ...
        if (warn) {
            KrudWorld.error("Can't find block data for " + s);
        }
        return null;
    }
    ...
    private static BlockData parseBlockData(String ix, boolean warn) {
        try {
            ...
            if (!ix.startsWith("minecraft:") && ix.contains(":")) {
                ...
                Optional<BlockData> bd = KrudWorld.service(ExternalDataSVC.class).getBlockData(key);
                KrudWorld.debug("Loading block data " + key);
                if (bd.isPresent())
                    bx = bd.get();
            }
            ...
            if (bx instanceof Leaves && KrudWorldSettings.get().getGenerator().isPreventLeafDecay()) {
            ...
            if (clw.flip()) {
                KrudWorld.warn("Unknown Block Data: " + ix);
            }
            ...
            KrudWorld.debug("Converting " + ix + " to " + newBlock);
    ...
    public synchronized static String[] getBlockTypes() {
        ...
        for (Identifier id : KrudWorld.service(ExternalDataSVC.class).getAllIdentifiers(DataType.BLOCK))
            bt.add(id.toString());
        bt.addAll(custom.k());
    ...
    public synchronized static KMap<List<String>, List<BlockProperty>> getBlockStates() {
        ...
        for (var pair : KrudWorld.service(ExternalDataSVC.class).getAllBlockProperties()) {
        ...
        flipped.forEach((k, v) -> {
            ...
            if (old != null) KrudWorld.error("Duplicate block state: " + v + " (" + old + " and " + k + ")");
        });
    ...
    public static String[] getItemTypes() {
        ...
        for (Identifier id : KrudWorld.service(ExternalDataSVC.class).getAllIdentifiers(DataType.ITEM))
            bt.add(id.toString());
