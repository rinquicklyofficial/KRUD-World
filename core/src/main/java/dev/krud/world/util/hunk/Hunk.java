...
package dev.krud.world.util.hunk;

import dev.krud.world.KrudWorld;
import dev.krud.world.engine.object.KrudWorldPosition;
import dev.krud.world.util.collection.KList;
import dev.krud.world.util.function.*;
import dev.krud.world.util.hunk.storage.*;
import dev.krud.world.util.hunk.view.*;
...
public interface Hunk<T> {
    ...
    static KrudWorldPosition rotatedBounding(int w, int h, int d, double x, double y, double z) {
        int[] iii = {0, 0, 0};
        int[] aaa = {w, h, d};
        int[] aai = {w, h, 0};
        int[] iaa = {0, h, d};
        ...
        return new KrudWorldPosition(maxX - minX, maxY - minY, maxZ - minZ);
    }
    ...
    default void set(int x, int y, int z, T t) {
        if (!contains(x, y, z)) {
            KrudWorld.warn("OUT OF BOUNDS " + x + " " + y + " " + z + " in bounds " + getWidth() + " " + getHeight() + " " + getDepth());
            return;
        }
        ...
        for (int i = 0; i < t.getWidth(); i++) {
            for (int j = 0; j < t.getHeight(); j++) {
                for (int k = 0; k < t.getDepth(); k++) {
                    t.set(i, j, k, interpolated.fromDouble(KrudWorldInterpolation.getNoise3D(d, i, j, k, scale, n3)));
                }
            }
        }
    ...
    default Hunk<T> interpolate2D(double scale, InterpolationMethod d, Interpolated<T> interpolated) {
        ...
        for (int i = 0; i < t.getWidth(); i++) {
            for (int j = 0; j < t.getDepth(); j++) {
                t.set(i, 0, j, interpolated.fromDouble(KrudWorldInterpolation.getNoise(d, i, j, scale, n2)));
            }
        }
