...
import dev.krud.world.util.reflect.V;
...
public abstract class VolmitPlugin extends JavaPlugin implements Listener {
    ...
    private void registerCommands() {
        ...
        for (Field i : getClass().getDeclaredFields()) {
            if (i.isAnnotationPresent(dev.krud.world.util.plugin.Command.class)) {
                try {
                    i.setAccessible(true);
                    MortarCommand pc = (MortarCommand) i.getType().getConstructor().newInstance();
                    dev.krud.world.util.plugin.Command c = i.getAnnotation(dev.krud.world.util.plugin.Command.class);
    ...
    public void registerListener(Listener l) {
        KrudWorld.debug("Register Listener " + l.getClass().getSimpleName());
