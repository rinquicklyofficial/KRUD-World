...
package dev.krud.world.util.profile;
...
public class LoadBalancer extends MsptTimings {
    ...
    public LoadBalancer(Semaphore semaphore, int maxPermits, KrudWorldSettings.MsRange range) {
    ...
    protected void update(int raw) {
        ...
        int mspt = raw;
        if (mspt < lastMspt) {
            int min = (int) Math.max(lastMspt * KrudWorldSettings.get().getUpdater().getChunkLoadSensitivity(), 1);
        ...
        if (diff == 0) return;
        KrudWorld.debug("Adjusting load to %s (%s) permits (%s mspt, %.2f)".formatted(target, diff, raw, percent));
    ...
    public void setRange(KrudWorldSettings.MsRange range) {
