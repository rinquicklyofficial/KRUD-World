...
import dev.krud.world.util.decree.DecreeParameter;
...
public class VolmitSender implements CommandSender {
    ...
    public void sendTitle(String title, String subtitle, int i, int s, int o) {
        KrudWorld.audiences.player(player()).showTitle(Title.title(
    ...
    public void sendAction(String action) {
        KrudWorld.audiences.player(player()).sendActionBar(createNoPrefixComponent(action));
    }

    public void sendActionNoProcessing(String action) {
        KrudWorld.audiences.player(player()).sendActionBar(createNoPrefixComponentNoProcessing(action));
    }
    ...
    private Component createNoPrefixComponent(String message) {
        if (!KrudWorldSettings.get().getGeneral().canUseCustomColors(this)) {
        ...
        String a = C.aura(t, KrudWorldSettings.get().getGeneral().getSpinh(), KrudWorldSettings.get().getGeneral().getSpins(), KrudWorldSettings.get().getGeneral().getSpinb(), 0.36);
    ...
    private Component createComponent(String message) {
        ...
        String a = C.aura(t, KrudWorldSettings.get().getGeneral().getSpinh(), KrudWorldSettings.get().getGeneral().getSpins(), KrudWorldSettings.get().getGeneral().getSpinb());
        return MiniMessage.miniMessage().deserialize(a);
    }
    ...
    public void sendMessage(String message) {
        ...
        if ((!KrudWorldSettings.get().getGeneral().isUseCustomColorsIngame() && s instanceof Player) || !KrudWorldSettings.get().getGeneral().isUseConsoleCustomColors()) {
        ...
        try {
            KrudWorld.audiences.sender(s).sendMessage(createComponent(message));
        } catch (Throwable e) {
            String t = C.translateAlternateColorCodes('&', getTag() + message);
            String a = C.aura(t, KrudWorldSettings.get().getGeneral().getSpinh(), KrudWorldSettings.get().getGeneral().getSpins(), KrudWorldSettings.get().getGeneral().getSpinb());

            KrudWorld.debug("<NOMINI>Failure to parse " + a);
            s.sendMessage(C.translateAlternateColorCodes('&', getTag() + message));
        }
    }
    ...
    public void sendMessageRaw(String message) {
        ...
        try {
            KrudWorld.audiences.sender(s).sendMessage(createComponentRaw(message));
        } catch (Throwable e) {
            String t = C.translateAlternateColorCodes('&', getTag() + message);
            String a = C.aura(t, KrudWorldSettings.get().getGeneral().getSpinh(), KrudWorldSettings.get().getGeneral().getSpins(), KrudWorldSettings.get().getGeneral().getSpinb());
            ...
            sendMessageRaw(helpCache.computeIfAbsent(i.getPath(), (k) -> {
                ...
                if (i.isNode()) {
                    for (DecreeParameter p : i.getNode().getParameters()) {
                        ...
                        String nUsage;
                        String fullTitle;
                        KrudWorld.debug("Contextual: " + p.isContextual() + " / player: " + isPlayer());
