...
package dev.krud.world.util.decree.virtual;
...
import dev.krud.world.util.decree.*;
...
import dev.krud.world.util.plugin.CommandDummy;
...
public class VirtualDecreeCommand {
    ...
    private boolean invokeTabComplete(KList<String> args, KList<Integer> skip, KList<String> tabs, String raw) {
        if (isStudio() && !KrudWorldSettings.get().getStudio().isStudio()) {
            return false;
        }
    ...
    private KMap<String, Object> map(VolmitSender sender, KList<String> in) {
        ...
        for (int x = 0; x < knownInputs.size(); x++) {
            ...
            if (param == null) {
                KrudWorld.debug("Can't find parameter key for " + key + "=" + value + " in " + getPath());
            ...
            try {
                ...
                KrudWorld.debug("Can't parse parameter value for " + key + "=" + value + " in " + getPath() + " using handler " + param.getHandler().getClass().getSimpleName());
        ...
        for (int x = 0; x < unknownInputs.size(); x++) {
            ...
            try {
                ...
                try {
                    ...
                    KrudWorld.debug("Can't parse parameter value for " + par.getName() + "=" + stringParam + " in " + getPath() + " using handler " + par.getHandler().getClass().getSimpleName());
    ...
    public boolean invoke(VolmitSender sender, KList<String> args, KList<Integer> skip) {
        if (isStudio() && !KrudWorldSettings.get().getStudio().isStudio()) {
            sender.sendMessage(C.RED + "To use KrudWorld Studio Commands, please enable studio in KrudWorld/settings.json (settings auto-reload)");
            return false;
        }
        ...
        KrudWorld.debug("@ " + getPath() + " with " + args.toString(", "));
        if (isNode()) {
            KrudWorld.debug("Invoke " + getPath() + "(" + args.toString(",") + ") at ");
    ...
    private boolean invokeNode(VolmitSender sender, KMap<String, Object> map) {
        ...
        for (DecreeParameter i : getNode().getParameters()) {
            ...
            try {
                ...
                KrudWorld.debug("Can't parse parameter value for " + i.getName() + "=" + i.getParam().defaultValue() + " in " + getPath() + " using handler " + i.getHandler().getClass().getSimpleName());
            ...
            if (sender.isPlayer() && i.isContextual() && value == null) {
                ...
                if (ch != null) {
                    ...
                    if (value != null) {
                        KrudWorld.debug("Parameter \"" + i.getName() + "\" derived a value of \"" + i.getHandler().toStringForce(value) + "\" from " + ch.getClass().getSimpleName());
                    } else {
                        KrudWorld.debug("Parameter \"" + i.getName() + "\" could not derive a value from \"" + ch.getClass().getSimpleName());
                    }
                } else {
                    KrudWorld.debug("Parameter \"" + i.getName() + "\" is contextual but has no context handler for \"" + i.getType().getCanonicalName() + "\"");
                }
            }

            if (i.hasDefault() && value == null) {
                try {
                    KrudWorld.debug("Parameter \"" + i.getName() + "\" is using default value \"" + i.getParam().defaultValue() + "\"");
                    value = i.getDefaultValue();
                } catch (Throwable e) {
                    e.printStackTrace();
                }
